...
export default function Ranking() {
  const [activeTab, setActiveTab] = useState('popularProducts');
  ...
  const renderPopularSearchesList = (rankingData) => (
      <div className="divide-y divide-gray-200">
        {rankingData.map((item) => (
            <div key={item.rank}
                 className={`flex flex-col sm:flex-row sm:items-center sm:justify-between p-6 transition-colors duration-200 hover:bg-gray-50`}>
              <div className="flex items-center mb-4 sm:mb-0">
                <div className={`text-2xl font-bold text-gray-500 w-12 text-center ${item.rank <= 3 ? `text-4xl` : ``}`}>{getRankIcon(item.rank)}</div>
                <div className="ml-4">
                  <Link href={`/products?search=${encodeURIComponent(item.keyword)}`}>
                    <div className="text-lg font-bold text-gray-800 hover:text-accent transition-colors duration-200 flex items-center gap-2">
                      {item.keyword}
                    </div>
                  </Link>
                </div>
  ...
  const renderPopularProductsList = (rankingData) => (
      <div className="divide-y divide-gray-200">
        {rankingData.map((item) => (
            <div key={item.rank}
                 className={`flex flex-col sm:flex-row sm:items-center sm:justify-between p-6 transition-colors duration-200 hover:bg-gray-50`}>
                ...
                <div className="ml-4 flex items-center gap-3 flex-1 min-w-0">
                  {item.imageUrl ? (
                      <Image
                          ...
                          className="object-cover rounded-lg border border-gray-200 flex-shrink-0 w-12 h-12"
                      />
                  ) : (
                       <Image
                  ...
                  <div className="min-w-0">
                    <Link href={`/products/${item.productId}`} target={'_blank'}>
                      <div className="text-lg font-bold text-gray-800 hover:text-accent transition-colors duration-200 flex items-center gap-1">
                    ...
                    <div className="text-sm text-gray-500 mt-1">{item.categoryName}</div>
                  </div>
                </div>
              </div>
              <div className="text-2xl font-bold text-primary pl-16 sm:pl-0">
                {item.minPrice ? `${item.minPrice.toLocaleString()}원` : '-'}
              </div>
            </div>
        ))}
      </div>
  ...
  const tabs = [
    {id: 'popularProducts', label: '🔥 인기 상품'},
    {id: 'popularSearches', label: '🔍 인기 검색어'},
                ...
                <div className="p-8 text-center">
                  ...
                  <>
                    {activeTab === 'popularProducts' &&
                        renderTabContent('인기 상품 TOP 20', '최근 한 달간 구매 클릭이 가장 많은 상품들', renderPopularProductsList(rankingData.popularProducts.length > 0 ? rankingData.popularProducts : []))}
                    {activeTab === 'popularSearches' &&
                        renderTabContent('인기 검색어 TOP 20', '최근 한 달간 가장 많이 검색된 키워드', renderPopularSearchesList(rankingData.popularSearches.length > 0 ? rankingData.popularSearches : []))}
                    {activeTab === 'products' &&
                        renderTabContent('판매 사이트별 상품 보유 현황', '등록된 전체 상품 중 각 사이트의 보유 비율', renderSiteRankingList(rankingData.productCount.length > 0 ? rankingData.productCount : [], 'products'))}
                    {activeTab === 'prices' &&
                        renderTabContent('최저가 상품 개수', '동일 상품군에서 최저가를 제공한 횟수 기준', renderSiteRankingList(rankingData.lowestPrice.length > 0 ? rankingData.lowestPrice : [], 'prices'))}
                    {activeTab === 'avgPrice' &&
                        renderTabContent('사이트별 상품 평균 가격', '사이트별 등록된 상품들의 평균 판매 가격 (낮은 순)', renderAveragePriceRankingList(rankingData.averagePrice.length > 0 ? rankingData.averagePrice : []))}
                  </>
              )}
        </div>
      </div>
