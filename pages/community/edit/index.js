...
// Import Froala Editor CSS (these can be imported statically)
import 'froala-editor/css/froala_style.min.css';
import 'froala-editor/css/froala_editor.pkgd.min.css';

// Dynamic import of FroalaEditorComponent with SSR disabled
...
export default function PostEditPage() {
  ...
  const froalaConfig = {
    ...
    pluginsEnabled: ['align', 'charCounter', 'codeBeautifier', 'codeView', 'colors', 'draggable', 'emoticons', 'entities', 'fontFamily', 'fontSize', 'fullscreen', 'image', 'inlineStyle', 'lineBreaker', 'lineHeight', 'link', 'lists', 'paragraphFormat', 'paragraphStyle', 'quickInsert', 'quote', 'save', 'table', 'url', 'video', 'wordPaste'],
    imageUploadURL: '/api/community/posts/upload-image',
    imageUploadParams: {},
    imageUploadMethod: 'POST',
    imageMaxSize: 5 * 1024 * 1024, // 5MB
    imageAllowedTypes: ['jpeg', 'jpg', 'png', 'gif', 'webp', 'bmp'],
    imageMultipleFiles: true, // 다중 이미지 선택 허용
    imageManagerToggleTags: false,
    videoUploadURL: '/api/community/posts/upload-video',
    videoUploadParams: {},
    videoUploadMethod: 'POST',
    videoMaxSize: 50 * 1024 * 1024, // 50MB
    videoAllowedTypes: ['webm', 'jpg', 'ogg', 'mp4', 'gif'],
    events: {
      ...
      'froalaEditor.image.uploaded': function (e, editor, response) {
        ...
        if (data.link) {
          ...
          toast.success(`${data.links.length}개의 이미지가 업로드되었습니다.`);
        } else {
      ...
      'froalaEditor.video.uploaded': function (e, editor, response) {
        ...
        if (data.link) {
          ...
          toast.success(`${data.links.length}개의 비디오가 업로드되었습니다.`);
        } else {
          toast.error('비디오 업로드 실패: ' + (data.message || '알 수 없는 오류'));
        }
      },
      'froalaEditor.video.error': function (e, editor, error) {
