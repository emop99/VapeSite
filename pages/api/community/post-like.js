...
import {Like, Notification, NotificationSettings, Post, User} from '../../../models';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'like') {
      ...
      if (created) {
        try {
          // 자신의 게시글에 좋아요를 누른 경우는 알림 생성하지 않음
          if (post.userId !== user.id) {
            ...
            if (!authorSettings || authorSettings.likeEnabled) {
              // 알림 생성
              const notificationContent = `${user.nickName}님이 회원님의 게시글을 좋아합니다.`;
              ...
              // 실시간 소켓 알림 전송 (가능한 경우)
              try {
                const sendResult = sendRealTimeNotification(post.userId, {
                  type: 'like',
                  content: notificationContent,
                  url: `/community/post/${post.id}`,
                });
