...
// 외부 스크립트 컴포넌트 (GTM, Clarity, AdSense)
const ExternalScripts = () => {
  const { data: session } = useSession();
  const router = useRouter();

  // 개발 환경이거나 관리자 페이지 경로는 제외
  if (process.env.NODE_ENV === 'development' || router.pathname.startsWith('/admin')) {
    return null;
  }
