...
export default function ProductList() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error('카테고리를 불러오는데 문제가 발생했습니다.');
        }
        ...
        // 카테고리 로딩 실패 시 에러 표시하지 않고 빈 배열로 설정
        setCategories([]);
      } finally {
  ...
  // 제품 그룹화 (카테고리별로 그룹화하지 않고 단일 목록으로 표시)
  // 무한 스크롤을 위해 단일 목록으로 처리
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          ...
          {loadingCategories ? (
            <div className="px-4 py-2 animate-pulse text-gray-500">
              카테고리 로딩 중...
            </div>
          ) : (
      ...
      {products.length > 0 ? (
        <div className="mb-12">
          {activeCategory !== 'all' && (
            <h2 className="text-2xl font-bold mb-4 pb-2 border-b">{activeCategory}</h2>
          )}
