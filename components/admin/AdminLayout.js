...
import { FiUsers, FiBox, FiShoppingBag, FiBarChart2, FiSettings, FiMenu, FiLogOut, FiUser } from 'react-icons/fi';
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-goblin-dark text-white shadow-lg transition-all duration-300 overflow-hidden fixed h-full z-20`}>
        ...
        <nav className="mt-6">
          ...
          <Link href="/admin" className={`flex items-center px-4 py-3 ${isActive('/admin') && !isActive('/admin/users') && !isActive('/admin/products') && !isActive('/admin/manufacturers') ? 'bg-goblin-light text-white font-bold' : 'text-gray-300 hover:bg-goblin-light'}`}>
            <FiBarChart2 className="h-5 w-5" />
            {sidebarOpen && <span className="ml-3">대시보드</span>}
          </Link>

          <Link href="/admin/products" className={`flex items-center px-4 py-3 ${isActive('/admin/products') ? 'bg-goblin-light text-white font-bold' : 'text-gray-300 hover:bg-goblin-light'}`}>
            <FiBox className="h-5 w-5" />
            {sidebarOpen && <span className="ml-3">상품 관리</span>}
          </Link>

          <Link href="/admin/manufacturers" className={`flex items-center px-4 py-3 ${isActive('/admin/manufacturers') ? 'bg-goblin-light text-white font-bold' : 'text-gray-300 hover:bg-goblin-light'}`}>
            <FiShoppingBag className="h-5 w-5" />
            {sidebarOpen && <span className="ml-3">제조사 관리</span>}
          </Link>

          <Link href="/admin/users" className={`flex items-center px-4 py-3 ${isActive('/admin/users') ? 'bg-goblin-light text-white font-bold' : 'text-gray-300 hover:bg-goblin-light'}`}>
